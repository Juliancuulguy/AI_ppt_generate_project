...
openai.api_key='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'

os.environ['STABILITY_HOST'] = 'grpc.stability.ai:443'
os.environ['STABILITY_KEY'] = 'sk-EHY6qkXvUrtEorzGcqgDhw6XKsrOJcUFd4EHeCmQZu5EbFSV'
...
def generate_ppt():
    ...
    chatans.clear()
    prompts = f'我是學生，我想製作一個PPT，主題是關於{key_word}，請以一簡短句生成副標題'
...
def generate_one_page_ppt():
    ...
    if pptchoose == '6':
        ...
        RIGHT_TRIANGLE_2.fill.fore_color.rgb = RGBColor(sc1+20, sc2+20, sc3+20)
        #RIGHT_TRIANGLE_2.line
        RIGHT_TRIANGLE_2.line.color.rgb = RGBColor(sc1+20, sc2+20, sc3+20)
    ...
    if pptchoose == '9':
        ...
        rectangle_3.fill.fore_color.rgb = RGBColor(sc1, sc2, sc3)
        #rectangle.line
        rectangle_3.line.color.rgb = RGBColor(sc1, sc2, sc3)
        ...
        rectangle_5.fill.fore_color.rgb = RGBColor(sc1, sc2, sc3)
        #rectangle.line
        rectangle_5.line.color.rgb = RGBColor(sc1, sc2, sc3)
...
class PageOne(tk.Frame):  #chatGPT
    ...
    def __init__(self, master, *args, **kwargs):
        ...
        def generate_titles():
            global spl,chatans,key_word,tousr, amounts, spl_copy
            ...
            #     print(key_words)
            prompts = f'我想製作一個介紹{key_word}的簡報，請幫我用繁體中文生成{amounts}個跟{key_word}相關的標題'
            doQA(prompts, 512, 0.5)
            spp = ''.join(chatans[0])
            spl = spp.split('\n')
            tousr = ""
...
#下一頁按鈕
        self.button2 = ctk.CTkButton(
            ...
            hover_color='#8082AC',
            bg_color="#F1F2D9",
            command=lambda: [save_spl(), master.switch_Canvas(PageTwo),generate_ppt(),hint_to_0() ],
            font=('Aries', 20),
        ...
        self.button3 = ctk.CTkButton(
            ...
            text_color= 'white', #文字顏色
            bg_color="#F1F2D9", 
            command=lambda: [generate_titles(),give_hint()],
            fg_color='#7B7DB7', #按鈕顏色
...
#下一頁按鈕
        self.button2 = ctk.CTkButton(
            ...
            hover_color='#8082AC',
            bg_color="#F1F2D9",
            command=lambda: [next(), clearContents(), master.switch_Canvas(temp), check_choose_page()],
            font=('Aries', 20),
            corner_radius= 0, #邊角圓弧度
            width=98.0,
            height=50.0
...
class temp(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        def clearContents():
            global page_of_ppt1, spl
            if page_of_ppt1 < len(spl):
                self.entry3.delete('-1', tk.END)
                self.entry2.delete('1.0', tk.END)
                self.entry1.delete('1.0', tk.END)
                self.entry4.delete('1.0', tk.END)
                self.entry3.insert(END, spl[page_of_ppt1])
            
        def check_contents():
            global spp2,spl2,try1,spl, tindex, contents_done, spp3, cont, spl_copy, consave
        ...
        def generate_contents():
            global chatans,tousr,spp2,page_of_ppt1, cont, spp3, note
            note = ''
            spp2 = ''
            spp3.clear()
            chatans.clear()
            ...
            if cont == 1:
                if self.combo.get() == "依我提供的資料擴充生成內文":
                    word = self.entry1.get(1.0, "end")
                    prompts = f'''我要製作一頁有關"{ttopic}"的簡報，請擴充生成包括以下提供的文句："{word}"並且適合放進這一頁簡報的內文，總共一句。'''
                    ...
                    prompts = f'''我要製作一頁有關"{ttopic}"的簡報，請擴充生成包含以下提供的文句："{spp2}"的專業的詳細的介紹，總共一段。'''
                    spp3.clear()
                    chatans.clear()
                    doQA(prompts, 2048, 0.5)
                    note = ''.join(chatans[0])
                    ...
                    if self.combo.get() == "依我提供的資料縮減生成內文":
                        word = self.entry1.get(1.0, "end")
                        prompts = f'''我要製作一頁有關"{ttopic}"的簡報，請用繁體中文摘錄"{word}"這段文字的重點，總共一段'''
                    elif self.combo.get() == "生成全新的內文":
                        prompts = f'''我要製作一頁有關"{ttopic}"的簡報，
                        你可以試著回答"{ttopic}"這個問題，生成對這個問題的解答；
                        或是詳細解釋"{ttopic}"是什麼；
                        或是詳細回答如何解決"{ttopic}"這件事；
                        或是舉出一個跟"{ttopic}"這件事有關的實際例子；
                        或是提供相應主題的數據和統計資料，以數字化的方式呈現相關信息；
                        或是引用相關領域的專家見解和研究，生成對應的簡報內文講稿；
                        或是提供對應主題的具體行動計畫和建議；
                        或是生動描述相應主題的重要性和影響；
                        或是探討相應主題的未來發展趨勢和可能的創新方向；
                        或是生成一個與對應標題相關的真實情境故事；
                        或是分析對應標題所指涉的領域的趨勢，包括當前狀態、未來預測，以及可能的影響；
                        或是比較相關主題的不同方面，並突顯它們之間的異同；
                        或是呈現與標題相關的有趣統計數據，以吸引聽眾的注意；
                        或是探討標題所指涉的主題對環境的影響；
                        或是描述與標題相關主題的歷史演變，探討它是如何發展和改變的；
                        或是探討與標題相關的領域未來的可能發展，包括趨勢、新興技術或社會變革；

                        請自行判斷哪個回答方向最適合"{ttopic}"這個標題，然後生成對應的簡報內文講稿。麻煩請用繁體中文生成，總共一段，中間請勿分段'''
                    doQA(prompts, 2048, 0.5)
                    note = ''.join(chatans[0])
                    print(note)
                    note = clean(note, True)
                    ...
                    for i in range(len(spp3)):
                        if len(spp3[i])>6:
                            ...
                            note = spp3[i]
                    self.entry4.insert('1.0', note)
                    prompts = f'''我要製作一頁有關"{ttopic}"的簡報，請從"{note}"這段文字摘取重點，總共一句'''
                    spp3.clear()
                    chatans.clear()
                    doQA(prompts, 2048, 0.5)
                    spp2 = ''.join(chatans[0])
                ...
                if self.combo.get() == "依我提供的資料擴充生成內文":
                    ...
                    if self.combo.get() == "依我提供的資料縮減生成內文":
                        ...
                        請自行判斷哪個回答方向最適合"{ttopic}"這個標題，然後生成對應的簡報內文講稿。麻煩請用繁體中文生成，總共{cont}段'''
                    doQA(prompts, 2048, 0.5)
                    note = ''.join(chatans[0])
                    print(note)
                    note = clean(note, True)
        ...
        self.button2 = ctk.CTkButton(
            ...
            hover_color='#8082AC',
            bg_color="#F1F2D9",
            command=lambda: [next1(), clearContents(), piccornot(), finishornot(),hint_to_0()],
            font=('Aries', 20),
            corner_radius= 0, #邊角圓弧度
            width=98.0,
            height=50.0
...
#內文output
        ...
        self.button5 = ctk.CTkButton(
            ...
            hover_color='#8082AC',
            bg_color="#F1F2D9",
            command=lambda: [generate_contents(),give_hint()],
            font=('Aries', 16),
            corner_radius= 30, #邊角圓弧度
            width=80.0,
            height=35.0,
...
class PageThree(tk.Frame):  #stable diffusion
    ...
    def __init__(self, master, *args, **kwargs):
        ...
        def generate_pict():
            ...
            print("c")
            chatans.clear()
            key_words = "((master piece)),((high quality)),ultra-detailed, illustration, " + key_words
        ...
        def inittext():
            self.button1.configure(text = 'choose(A)')
            self.button2.configure(text = 'choose(B)')
            self.button4.configure(text = 'choose(C)')
        ...
        def hint_to_0():
            ...
            self.canvas.create_text(
                ...
                10.0,
                anchor="nw",
                text="請按choose鈕選擇要放進簡報的圖片，如果都不滿意可以按「生成圖片」重新生成",
                fill="#FFFFFF",
                font=("InriaSans Regular", 20 * -1),
                tags= 'origin'
            )
        ...
        self.button6 = ctk.CTkButton(
            ...
            hover_color='#8082AC',
            bg_color="#F1F2D9",
            command=lambda: [master.switch_Canvas(PageTwo), finishornot(), inittext()],
            font=('Aries', 20),
            corner_radius= 0, #邊角圓弧度
            width=98.0,
            height=50.0
